...
namespace UserManagement.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var lstUsers = await userManager.GetUsersInRoleAsync(role.Name);

            var lstUserIds = lstUsers.Select(x => x.Id.ToLowerInvariant());

            var allUserIds = userManager.Users.Select(x => x.Id.ToLowerInvariant());

            List<User> members = userManager.Users.Where(x => lstUserIds.Contains(x.Id.ToLowerInvariant())).ToList();
            List<User> nonMember = userManager.Users.Where(x => !lstUserIds.Contains(x.Id.ToLowerInvariant())).ToList();
